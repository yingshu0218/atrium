...
框架（Framework）：模块系统、宿主、数据访问、认证、同步、UI 外壳、主题、Agent 通道等通用能力。

应用（Application）：基于 Atrium 组合配置和模块形成的独立项目，例如个人工作台。

部署实例（Deployment Instance）：某个应用的一次实际部署，拥有独立运行环境和数据。

客户端（Client）：连接部署实例的浏览器、PWA、桌面端、移动端或 Agent。
